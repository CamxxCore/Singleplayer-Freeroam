...
namespace SPFServer.Main
{
    internal class GameManager
    {
        ...
        private List<GameVehicle> activeVehicles;
        ...
        internal GameManager()
        {
            ...
            activeVehicles = new List<GameVehicle>();
        }
            ...
            float damage, int targetID, out GameClient victim, out bool wasKilled)
        {
            ...
            for (int i = 0; i < gameState.Clients.Length; i++)
            {
                ...
                {
                    foreach (var cl in gameState.Clients)
                    {
                        //  dont want the killer
                        if (cl.ClientID == gameState.Clients[i].ClientID) continue;

                        // check distance
                        var dist = cl.Position.DistanceTo(impactCoords);

                        if (dist > 0.87f) continue;

                        // make sure the target exists
                        if (ClientFromID(cl.ClientID, out target) && target.State.Health > -1)
                        {
                            // modify health
        ...
        /// </summary>
        /// <returns></returns>
        internal ClientState[] GetClientStates(bool onlyRecent = true)
        {
            if (onlyRecent)
        ...
        /// </summary>
        /// <returns></returns>
        internal VehicleState[] GetVehicleStates(bool onlyRecent = true)
        {
            if (onlyRecent)
